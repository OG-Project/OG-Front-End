...
export const conexaoBD = defineStore('conexaoBD', {
    ...
    actions: {
      ...
      procurar(textoRequisicao){
        ...
        return axios.get("http://10.4.96.17:8084"+ textoRequisicao).then(response => response.data)
      },
      cadastrar(objeto, textoRequisicao){
        return axios.post("http://10.4.96.17:8084"+textoRequisicao,objeto)
      },
      atualizar(objeto,textoRequisicao){
        return axios.put("http://10.4.96.17:8084"+textoRequisicao,objeto)
      },
      adicionaUsuarioAEquipe(userId, equipeId, textoRequisicao){
        return axios.patch("http://10.4.96.17:8084"+textoRequisicao,userId,equipeId)
      },
      adicionarUsuarios(ids,equipeId,textoRequisicao){
        return axios.patch(`http://10.4.96.17:8084${textoRequisicao}/${equipeId}`, ids)
      },
      deletarEquipe(id,textoRequisicao){
        return axios.delete(`http://10.4.96.17:8084${textoRequisicao}/${id}`)
      },
      async buscarMembrosEquipe(equipeId,textoRequisicao){
          return await ((await axios.get(`http://10.4.96.17:8084${textoRequisicao}/${equipeId}`)).data)
      },
