      ...
      <h1 style="font-family:var(--fonteTitulo);font-size: var(--fonteTituloTamanho);" class="text-4xl" >{{ props.nomeDoTopico }}</h1>
        ...
        <div  v-for="projeto of listaDeProjetos" class="w-[100%] h-[12%] border-2 border-[var(--backgroundPuro)] bg-[var(--backgroundItems)] flex items-center justify-around"
          ...
          <div class="m-4 flex w-[70%] justify-between" style="border-bottom: 2px solid var(--roxo)" @click="redireciona('/projeto/kanban', projeto.id)">
            ...
            <p style="font-family:var(--fonteCorpo);" v-if="projeto.responsaveis[0]">{{ projeto.responsaveis[0].responsavel.username }}</p>
            <p style="font-family:var(--fonteCorpo);" v-if="!projeto.responsaveis[0]">{{ $t('home.naoPossuiResponsavel') }}</p>
          </div>
        </div>
      </div>
    </div>
