            ...
            <div class="bg-[var(--backgroundItems)] ml-8 w-[90%] h-[92%] flex items-center justify-end"
                ...
                <div class="overflow-y-auto h-[100%] w-[80%] flex items-center flex-col">
                    <div class="flex gap-12 mt-6 flex-wrap justify-center w-[100%] text-md">
                        ...
                        <div v-for="tarefa of tarefas" class="w-[80%] text-sm">
                            <div v-if="tarefa.nome">
                                <div class="flex gap-14">
                                    <p class="w-[16%] truncate flex items-center justify-center h-8 bg-[#B488D7]">
                                        {{ tarefa.nome }}</p>
                                    <p class="w-[16%] flex items-center justify-center h-8" v-if="tarefa.lider">{{
                            tarefa.lider }}</p>
                                    <p class="w-[16%] flex items-center justify-center h-8" v-else>Não possui</p>
                                    <p class="w-[16%] flex items-center justify-center h-8 bg-[#8A59B1]"
                                        v-if="tarefa.membros">{{ tarefa.membros.length }}</p>
                                    <p class="w-[16%] flex items-center justify-center h-8 bg-[#8A59B1]" v-else>Não
                                        possui</p>
                                    <p class="w-[16%] flex items-center justify-center h-8"
                                        :style="{ 'background-color': '#' + tarefa.status.cor }" v-if="tarefa.status">{{
                            tarefa.status.nome }}</p>
                                    <p class="w-[16%] flex items-center justify-center h-8 bg-[#93E28D]" v-else>Não
                                        possui</p>
                                    <p class="w-[16%] flex items-center justify-center h-8 bg-[#93E28D]"
                                        v-if="tarefa.horas">{{ tarefa.horas }}</p>
                                    <p class="w-[16%] flex items-center justify-center h-8 bg-[#93E28D]" v-else>Não
                                        possui</p>
                                    <p class="w-[16%] flex items-center justify-center h-8 bg-[#EF8F7A]"
