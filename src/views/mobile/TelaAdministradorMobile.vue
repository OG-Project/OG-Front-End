<template>
    <div class="h-[92vh] w-full">
        <div class="flex p-2 text-4xl justify-center">
            <h1 style="font-family:var(--fonteTitulo);">{{
                $t('paginaAdm.listaDeTarefas') }}</h1>
        </div>
        <div class="flex items-center mt-4 h-[38%] flex-col">
            <div class="bg-[var(--backgroundItems)] w-[95%] h-[92%] flex"
                style="box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px">
                <div class="overflow-y-auto h-[100%] w-[100%] flex justify-start"
                    id="scrollbar">
                    <div class="flex gap-4 mt-6 flex-wrap w-[100%] text-md flex">
                        <div class="flex gap-8 pb-8 w-[30%] items-center justify-center flex-col">
                            <p class="w-[100%] h-10 flex items-center justify-center"
                                style="font-family:var(--fonteCorpo);font-size:">{{
                $t('paginaAdm.nomeDaTarefa') }}</p>
                            <!-- <p class="w-[100%] h-10 flex items-center justify-center">Lider</p> -->
                            <!-- <p class="w-[100%] h-10 flex items-center justify-center">Membros</p> -->
                            <p class="w-[100%] h-10 flex items-center justify-center"
                                style="font-family:var(--fonteCorpo);font-size:">{{
                ...
                $t('paginaAdm.tempoTrabalhado') }}
                            ...
                            <!-- <p class="w-[100%] h-10 flex items-center justify-center">Entrega</p> -->
                            <p class="w-[100%] h-10 flex items-center justify-center"
                                style="font-family:var(--fonteCorpo);font-size:">{{
                ...
                $t('paginaAdm.historico') }}</p>
                        ...
                        <div v-for="tarefa of tarefas" class="w-[30%] text-sm flex-col gap-4">
                            <!-- <div class="h-full w-2 bg-white"></div> -->
                            <div v-if="tarefa.nome">
                                <div class="flex flex-col gap-8 w-[100%]">
                                    <p class="w-[100%] truncate flex items-center justify-center h-10 bg-[#B488D7] cursor-pointer"
                                        @click="redirecionamento('/criaTarefa', tarefa.id)">{{ tarefa.nome }}</p>
                                    <!-- <p class="w-[30%] flex items-center justify-center h-10" v-if="tarefa.lider"
                                        :style="{ color: corDaFonte(tarefa.status.cor) }">
                                        {{
                            tarefa.lider }}</p>
                                    <p class="w-[30%] flex items-center justify-center h-10"
                                        :style="{ color: corDaFonte(tarefa.status.cor) }" v-else>Não possui</p> -->
                                    <!-- <p class="w-[30%] flex items-center justify-center h-10 bg-[#8A59B1]"
                            ...
                            tarefa.membros.length }}</p>
                                    ...
                                    <p class="w-[100%] flex items-center justify-center h-10" v-if="tarefa.status"
                                        :style="{ 'background-color': '#' + tarefa.status.cor, color: corDaFonte(tarefa.status.cor) + 'font-family:var(--fonteCorpo);font-size:' }">
                                        {{ tarefa.status.nome }}</p>
                                    <p class="w-[100%] flex items-center justify-center h-10 bg-[#93E28D]"
                                        style="font-family:var(--fonteCorpo);font-size:"
                                        v-else>Não
                                        possui</p>
                                    <p class="w-[100%] flex items-center justify-center h-10 bg-[#93E28D]"
                ...
                tarefa.tempoAtuacao }}</p>
                                    <p class="w-[100%] flex items-center justify-center h-10 bg-[#93E28D]"
                                        ...
                                        v-else>Não
                                        possui</p>
                                    <!-- <p class="w-[100%] flex items-center justify-center h-10 bg-[#EF8F7A]"
                                        v-if="tarefa.diaCompleto">{{ format(new Date(tarefa.diaCompleto), "dd/MM/yyyy")
                                        }}</p>
                                    <p class="w-[100%] flex items-center justify-center h-10 bg-[#93E28D]" v-else>Não
                                        possui</p> -->
                                    <div class="w-[100%] flex items-center justify-center gap-10 h-10">
                                        <RejectedIcon class="w-[100%] h-[85%] cursor-pointer"
                                            @click="deixaTarefaConcluida(tarefa, 'Rejeitado')"></RejectedIcon>
                                        <AprovedIcon class="w-[100%] h-[60%] cursor-pointer"
                                            @click="deixaTarefaConcluida(tarefa, 'Aprovado')"></AprovedIcon>
                                    </div>
                                    <div class="w-[100%] flex items-center justify-center gap-10 h-10">
                                        <IconeHistorico @click="abrePopUp(tarefa, 'tarefa')"
                                            class="w-[100%] h-[85%] cursor-pointer">
                                        </IconeHistorico>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="flex relative w-full gap-8 mt-16 h-[38%]">
            <div class="flex pl-12 flex-col justify-center w-[100%] h-[85%] border-r-2 border-black">
    ...
    <HistoricoPopUp :texto-requisicao="textoRequisicao" :id="number.id" v-if="funcaoPopUp.variavelModal">
    </HistoricoPopUp>
</template>
<script setup>
